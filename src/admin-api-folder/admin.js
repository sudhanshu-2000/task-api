...
app.post("/add-payment-details-bank", verifytoken, (req, res) => {
  var body = req.body;
  con.query("select * from `new_payment_details` where `ac_no` = ?", [body.account_no], (err, result) => {
    if (err) throw err;
    if (result.length > 0) {
      res.status(302).json({
        error: true,
      ...
      con.query("INSERT INTO `new_payment_details`(`ac_holder_name`, `ac_no`, `ac_type`, `ifsc_code`, `bank_name`, `type`) VALUES (?,?,?,?,?,?)",
        [body.account_holder_name, body.account_no, body.account_type, body.ifsc_code, body.bank_name, body.type], (err, result) => {
          if (err) throw err;
          if (result) {
            res.status(200).json({
              error: false,
              status: true,
              massage: "Bank Details SuccessFully",
            });
          }
        }
      );
